...
Active targets (PENDING/SNIPING/WATCHING) have an **Edit** button on the card opening an inline editor for `partySize` + `preferredTimes`. **WATCHING targets** additionally get date inputs to edit the reservation `date` and range `dateEnd` (same "through" UI as Add Target; blank end clears the range). Saves via `PATCH /api/targets/[id]`. Click-order is preserved and newly-toggled times append — consistent with the no-auto-sort rule. The PATCH route validates `date`/`dateEnd` (parse, end ≥ start, span ≤ 14 days, single-day range collapses to null; `dateEnd: null` clears it). `snipeAt` is also PATCH-able server-side but not yet exposed in the UI.
...
Watch targets can span a **date range** via the optional nullable `dateEnd` column (`date` → `dateEnd` inclusive; null = single-day watch, fully backward compatible). Scheduled snipe is single-day only — a drop fires at one time for one date. On each cron tick, `watchDateStrings(target)` in `app/api/cron/snipe/route.ts` yields the `YYYY-MM-DD` days to check (from today or `date`, whichever is later, through `dateEnd`), chronological, **capped at a 14-day span** (`MAX_WATCH_SPAN_DAYS`) = 15 inclusive calendar days, to bound per-tick API calls against the single-IP scaling ceiling. The generator loop caps at `MAX_WATCH_SPAN_DAYS + 1` inclusive days so the cap matches the span validation (a span of N is N+1 days); both `POST /api/targets` and `PATCH /api/targets/[id]` import the same constant so the enforced limit and the checked limit can't drift. The watch loop (both Resy and OT) checks each day in order and **books the first day** with a matching preferred time → status BOOKED, target done (one reservation total). Expiry, the WATCH-fetch query, and auto-FAILED all key off the effective end (`dateEnd ?? date`), not `date`. The `POST /api/targets` route validates `dateEnd` (WATCH only): must parse, be ≥ `date`, span ≤ `MAX_WATCH_SPAN_DAYS`; a single-day range collapses to null. The Add Target modal shows a "through" end-date input in Watch mode; `TargetCard` renders `date – dateEnd`. The range is also editable inline on WATCHING targets (see Inline target editing above).
